...
const levels = [
    ...
    },
    {
        ...
        pipeSpawnInterval: 110,
        totalPipes: 18,
        description: "Expert - Pass 18 pipes (Smaller gap)"
    },
    {
        name: "Level 6",
        pipeSpeed: 4.5,
        ...
        pipeSpawnInterval: 110,
        totalPipes: 18,
        description: "Master - Pass 18 pipes (Tiny gap)"
    }
];
...
function createCoin() {
    ...
    const coinSize = 30; // Made coins bigger
    let coinY;
    let attempts = 0;
...
function drawBird() {
    ...
    if (needsAlpha) {
        ctx.save();
        ctx.globalAlpha = birdBlinkOn ? 0.3 : 1;
    }
    ...
    if (gameState === 'playing') {
        ...
        pipes.forEach(pipe => {
            ...
            if (!pipe.passed && pipe.x + pipeWidth < bird.x) {
                ...
                // Don't complete immediately - wait for finish line to be crossed
                // This ensures the finish line is created and the level ends properly
            }
        });
...
function checkCollisions() {
    ...
    // Check finish line collision - complete level when bird crosses finish line
    ...
    if (!isInvincible) {
        pipes.forEach(pipe => {
            ...
            if (bird.x < pipe.x + pipeWidth &&
                ...
                collidingPipe = pipe; // Store the pipe that caused the collision
                gameOver();
            }
...
function gameOver() {
    if (gameState === 'playing') {
        ...
        // Update coin display in game over popup
        updateCoinDisplay();
...
function completeLevel() {
    if (gameState === 'playing') {
        ...
        if (currentLevelIndex + 1 < levels.length) {
            ...
            // If it's the last level, hide the next level button
            if (nextLevelBtn) {
...
function goToNextLevel() {
    if (currentLevelIndex + 1 < levels.length) {
        ...
        // If it's the last level, return to menu
        returnToMenu();
    }
}
...
// Initialize game
// Ensure coins are initialized to 999999
...
levelCompletePopup.classList.add('hidden'); // Ensure level complete popup is hidden on startup
levelCompletePopup.style.display = 'none';
notEnoughCoinsElement.classList.add('hidden');
